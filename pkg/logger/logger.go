...
//NewOpts initialize a new log option, it will create the path if needed.
func NewOpts(path string) *Options {
	...
	if filepath.IsAbs(path) {
		...
		var err error
		opts.AbsLogDir, err = filepath.Abs(path)
		if err != nil {
			fmt.Printf("init log options failed, err is: %v", err)
			daemon.Exit(-1, err.Error())
		}
	}
	err := os.MkdirAll(opts.AbsLogDir, 0o700)
	if err != nil {
		daemon.Exit(-1, err.Error())
	}
	return opts
...
func EtcdClientLoggerConfig(opt *Options, filename string) *zap.Config {
...
func defaultEncoderConfig() zapcore.EncoderConfig {
	timeEncoder := func(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
		enc.AppendString(t.Format(utils.RFC3339Milli))
	}
...
func InitDefault(logOpt *Options) {
	encoderConfig := defaultEncoderConfig()
	...
	var gatewayCore zapcore.Core
	if logOpt.AbsLogDir != "" {
		lf, err := newLogFile(filepath.Join(logOpt.AbsLogDir, stdoutFilename), systemLogMaxCacheCount)
		if err != nil {
			daemon.Exit(1, err.Error())
		}
		...
		gatewayCore = zapcore.NewNopCore()
		gressLogger = zap.New(gatewayCore, opts...).Sugar()
	}
...
func initHTTPFilter(opt *Options) {
...
func initRestAPI(opt *Options) {
	restAPILogger = newPlainLogger(opt, adminAPIFilename, systemLogMaxCacheCount)
}

func newPlainLogger(opt *Options, filename string, maxCacheCount uint32) *zap.Logger {
